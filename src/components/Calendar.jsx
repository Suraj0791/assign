...
const Calendar = ({ onBack, onComplete, isSubmitting }) => {
  ...
  const moodColors = ['bg-red-500', 'bg-blue-500', 'bg-gray-400', 'bg-yellow-500', 'bg-green-500', 'bg-purple-500'];
        ...
        <div className="flex justify-center space-x-8">
          {moods.map((mood) => (
            <div key={mood.id} className="text-center">
              <div className={`w-12 h-12 ${mood.color} rounded-2xl flex items-center justify-center text-2xl mb-2`}>
                {mood.emoji}
              </div>
